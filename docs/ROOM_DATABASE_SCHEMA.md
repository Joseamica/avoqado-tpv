...
**Current Version:** 24 | **Database Name:** `avoqado_database` | **Location:** `/Users/amieva/Documents/Programming/Avoqado/avoqado-tpv/app/src/main/java/com/jaac/avoqado_tpv/core/data/local/AvoqadoDatabase.kt`

> **Canonical schema reference:** `app/schemas/com.jaac.avoqado_tpv.core.data.local.AvoqadoDatabase/<version>.json`
> (generated by Room at compile time since v24 — `exportSchema = true`). When writing a migration,
> copy the `createSql` from the JSON verbatim instead of hand-writing DDL. This doc is the
> human-readable companion; the JSON is what Room actually validates against.

## Entity Overview (12 Tables)
...
| `mosaic_shortcut` | Checkout shortcut tiles (Shortcuts/Configurar tabs) | Persistent per-venue config | N/A (persistent) |
| `angelpay_merchant_cache` | AngelPay user merchants (SDK 1.0.5 multi-merchant) | Cache-first + periodic refresh | Replaced on refresh |
...
| v5 → v6 | 2025-01-19 | Historical reports cache | Create `historical_periods` table ⚠️ created an extra index the entity never declared — repaired in v23→v24 |
...
| v12 → v13 | 2025-12-01 | Color support for products/categories | `product_categories.color` ⚠️ **BUG**: added `products.color` but ProductEntity declares `category_color` — mismatch crashed migrated devices on update ("Migration didn't properly handle: products"). Repaired in v23→v24 |
...
| v20 → v21 | 2026-05-08 | Checkout mosaic shortcuts | Create `mosaic_shortcut` table |
| v21 → v22 | 2026-05-18 | AngelPay multi-merchant offline cache | Create `angelpay_merchant_cache` table |
| v22 → v23 | 2026-05-29 | Offline queue idempotency | `pending_payments.idempotency_key` (nullable) |
| v23 → v24 | 2026-06-12 | **CRITICAL FIX** - Repair schema drift from v12→v13 and v5→v6 | Rebuild `products` to entity-exact DDL (drops orphan `color`, creates `category_color`); normalize `historical_periods` index set (drop orphan composite index). Both are pure caches — no payment data touched. Also enabled `exportSchema = true` (app/schemas/) |
